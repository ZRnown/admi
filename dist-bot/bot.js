...
class Bot {
    ...
    async processAndSend(message, tag) {
        ...
        const webhookName = isWebhook
            ? (message.webhook?.name || message.username || message.author?.username || "unknown")
            : "unknown";
        // 详细记录消息信息
        ...
        try {
            ...
            if (!isWebhook && authorId) {
                const allowed = (this.config.allowedUsersIds || []).map((x) => String(x)).filter(Boolean);
                const muted = (this.config.mutedUsersIds || []).map((x) => String(x)).filter(Boolean);
            ...
            if (kws.length > 0) {
                ...
                try {
                    for (const e of (message.embeds || [])) {
                        if (e.description)
                            pieces.push(String(e.description));
                        if (e.title)
                            pieces.push(String(e.title));
                        if (e.footer?.text)
                            pieces.push(String(e.footer.text));
                        if (e.author?.name)
                            pieces.push(String(e.author.name));
                        if (e.fields) {
                ...
                if (mapped) {
                    ...
                    if (senderForThis.webhookGuildId) {
                        ...
                        if (this.config.showSourceIdentity) {
                            // 显示源用户名称
                            display = ref.member?.displayName || ref.author?.username || ref.author?.tag || "用户";
                        }
                        else {
                            // 使用 webhook 名称
                            display = senderForThis.webhookName || "Webhook";
                        }
            ...
            try {
                if (isWebhook) {
                    ...
                    avatarUrl = message.webhook?.avatar
                        || message.avatarURL
                        || message.author?.displayAvatarURL?.({ size: 128, format: "png" })
                        || message.author?.avatarURL?.({ size: 128, format: "png" });
                }
                else {
                    // 普通用户消息
                    username = message.member?.displayName || message.author?.username || message.author?.tag;
                    const anyAuthor = message.author;
        ...
        if (results && results.length > 0) {
            ...
            if (first.sourceMessageId) {
                ...
                logMsg += `\n  内容: ${contentDisplay}`;
                if (hasAttachments)
                    logMsg += ` | 附件数: ${attachmentCount}`;
                if (hasEmbeds)
                    logMsg += ` | 嵌入: ${message.embeds.length}`;
                if (isReply)
                    logMsg += ` | 回复消息`;
                if (isWebhook)
                    logMsg += ` | Webhook消息`;
    ...
    async tryResolveMappingFromTarget(sourceId, senderForThis) {
        try {
            let configured = [];
            ...
            for (const channelId of configured) {
                try {
                    ...
                    while (unlimited || scanned < hardCap) {
                        ...
                        const batch = await ch.messages.fetch({ limit: step, ...(lastId ? { before: lastId } : {}) });
                        ...
                        if (arr.length < (unlimited ? 100 : Math.min(100, hardCap - scanned)))
                            break;
                    }
                }
                catch (e) {
    ...
    async messageAction(message, tag) {
        let render = "";
        const allAttachments = [];
        // 用户可见内容：仅进行 mention 渲染，不包含调试信息
        render += await this.renderMentions(message.content ?? "", message.mentions.users.values(), message.mentions.channels.values(), message.mentions.roles.values());
        // 精简日志：只在debug模式下记录基本信息，避免大量消息时I/O阻塞
        // 详细的embed和attachment信息已在processAndSend中记录
        return { content: render };
    }
    async renderMentions(text, users, channels, roles) {
